...
with panel_quality:
    ...
    st.caption('The curves show how well the model separates disease cases from healthy patients within each group, '
               ...
               'close together the model has similar discrimination across the two groups. A fairness gap can still '
               'appear at a particular decision threshold, because Equalised Odds depends on the true-positive and '
               'false-positive rates at that operating point, which points towards adjusting the threshold for each '
               'group. ROC is not one of the five fairness criteria. It is here to show where the '
...
with panel_compare:
    ...
    # Show the base rate gap for each dataset, which the fairness gaps vary alongside
    ...
    st.caption('This is the gap in disease rate between male and female patients within each dataset. The UCI data has '
               'a wide gap while the Kaggle data has almost none, so the two datasets provide contrasting cases in which '
               'the fairness trade-offs look very different. The datasets also differ in source and feature composition, '
               'so this comparison shows how the trade-offs vary alongside the base-rate gap rather than isolating it as '
               'a causal factor: the UCI data shows sharp trade-offs while the Kaggle data shows barely any. This gap '
